...
declare function pocappLib:getDBPediaMovieInfo($term1 as xs:string)
{

let $sterm1 := fn:replace($term1, " ", "_")


let $query := fn:concat("Select ?s ?p ?o where {BIND(<http://dbpedia.org/resource/",$sterm1,"_(film)> as ?s). 
                                             ?s ?p ?o.} LIMIT 100")
